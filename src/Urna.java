...
public class Urna {
  ...
  private static boolean votePresident(Voter voter) {
    ...
    // Branco
      ...
      int confirm = readInt();
      if (confirm == 1) {
        voter.vote(currentElection, "P", true, null);
        return true;
      } else
        votePresident(voter);
    } else {
      try {
        ...
        if (voteNumber == 0) {
          ...
          int confirm = readInt();
          if (confirm == 1) {
            voter.vote(currentElection, "P", false, null);
            return true;
          } else
            votePresident(voter);
        }
  ...
  private static boolean voteFederalDeputy(Voter voter, int counter) {
    ...
    if (vote.equals("br")) {
      ...
      int confirm = readInt();
      if (confirm == 1) {
        voter.vote(currentElection, "FederalDeputy", true, null);
        return true;
      } else
      ...
      try {
        ...
        if (voteNumber == 0) {
          ...
          int confirm = readInt();
          if (confirm == 1) {
            voter.vote(currentElection, "FederalDeputy", false, null);
            return true;
          } else
            return voteFederalDeputy(voter, counter);
        }
        ...
        ArrayList<Candidate> candidates = new ArrayList<Candidate>(currentElection.MAX);
        Candidate candidate1 = currentElection.getFederalDeputyByNumber(voter.state, voteNumber);
        candidates.add(candidate1);

        for (int i = 1; i < currentElection.MAX; i++) {
            String vote_ = readString();
            int voteNumber_ = Integer.parseInt(vote_);
            Candidate candidate_ = currentElection.getFederalDeputyByNumber(voter.state, voteNumber_);
